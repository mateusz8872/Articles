        ...
        <div class="row py-3">
            <div class="col-12 p-3">
                ...
                <div class="col-12"> 
                    ...
                    <a href="sorttop.php"><button type="button" class="m-1 btn btn-success float-right">Get by top 3 authors in last week</button></a>
                </div>
            </div>
        </div>
        <div class="row">
            ...
            $i=count($id);
            for($j = 0; $j < $i; $j++){
                $author='SELECT author_id,author FROM authors WHERE author_id='.$id[$j];
                $author_r= $conn->query($author);
                if ($author_r->num_rows > 0) {
                    while($row = $author_r->fetch_assoc()) {
                    echo "<div class='h3 col-12 float-left'> Author - " .$row["author"]."<br>
                    </div>";
                    }
                } else {
                        echo " <br>";
                }
                $sql = 'SELECT author_id FROM article_authors WHERE author_id ='. $id[$j];
                ...
                $sql1 = 'SELECT article.article_id,article.title,article.article,article.date,article_authors.article_id
                FROM article 
                LEFT JOIN article_authors
                ON article.article_id = article_authors.article_id
                LEFT JOIN authors
                ON authors.author_id = article_authors.author_id 
                WHERE article_authors.author_id='.$id[$j];
                $r = $conn->query($sql1);
                if ($r->num_rows > 0) {
                    while($row = $r->fetch_assoc()) {
                        ...
                        <div class='card col-12 my-1 table table-striped' id='selectedColumn''>
                            <div class='card-body'>
                                ...
                                <p class=' float-right col-5'>Date: ".$row["date"]."</p><br>
                                ...
                                <h5 class='float-left text-center col-12'>".$row["title"]."</h5>
                                <p class='float-left p-3 col-12 card-text'>".$row["article"]."</p>
                            </div>
                        </div>";
                    }   
                } else {
                echo "<h2 class=' col-12 float-left mt-2 mb-5'>This Author has no articles</h2>";
                }
            }
            ?>
        </div>
